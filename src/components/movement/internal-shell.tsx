...
import { InternalNav } from "@/components/movement/internal-nav";
import { SignOutButton } from "@/components/movement/sign-out-button";
  ...
  return (
    <div className="min-h-screen bg-background text-foreground lg:grid lg:grid-cols-[292px_1fr]">
      <aside className="sticky top-0 hidden h-screen border-r border-line bg-panel/88 p-4 backdrop-blur lg:block">
        <Link href="/movimento" className="mb-7 flex items-center gap-3 rounded-lg px-2 py-2 font-semibold">
          <LogoMark size="sm" />
          <span className="text-lg leading-tight">Movimento Hágios</span>
        </Link>
      ...
      <div className="min-w-0">
        <header className="sticky top-0 z-20 border-b border-line bg-background/92 px-4 py-3 backdrop-blur-xl lg:hidden">
          <div className="mb-3 flex items-center justify-between">
            <Link href="/movimento" className="flex items-center gap-2 font-semibold">
              <LogoMark size="xs" />
              Movimento Hágios
            </Link>
